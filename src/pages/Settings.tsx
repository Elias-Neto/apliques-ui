...
import { PermissionGroupCollapsible } from "@/components/PermissionGroupCollapsible"
import { usePermissions } from "@/hooks/use-permissions"
import { ModuleType, PermissionGroup } from "@/types/permissions"
  ...
  const handleApplyChanges = async () => {
    ...
    try {
      ...
      for (const groupId of allGroupsToUpdate) {
        ...
        await updatePermissions(groupId, payload)
      }
  ...
  if (error) {
    return (
      <div className="container py-8 space-y-8">
        ...
        <Card className="p-6">
          <p className="text-red-600">Erro ao carregar permissões: {error}</p>
        </Card>
      </div>
    )
  }
